...
export function createNumberStepInput(cell_class, input_class, key, value, defaultValue, minValue, maxValue, stepValue, limitValue) {
    const div = document.createElement('div');
    div.classList.add(cell_class);
    ...
    input.type = 'number';
    input.value = limitValue(value, defaultValue, minValue, maxValue, stepValue);
    input.setAttribute('defaultValue', defaultValue);
    input.min = minValue;
    input.max = maxValue;
    input.step = stepValue;
    input.addEventListener('change', () => {
        chrome.storage.local.set({ [key]: limitValue(input.value, defaultValue, minValue, maxValue, stepValue) });
    });
    div.appendChild(input);
